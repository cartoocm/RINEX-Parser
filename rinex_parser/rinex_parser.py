...
class RINEXHeader():
    def __init__(self, header):
        ...
        self.version, self.type, satellite = p.Parser(p.joint(
            n_ANY(9).parsecmap(float),
            p.compose(n_ANY(11), p.one_of('MON')),
            p.compose(n_ANY(19), n_ANY(1)))).parse(
                self.header_fields['RINEX VERSION / TYPE'])

    def _getMarkerInfo(self):
        self.marker_name = p.Parser(n_ANY(60)).parsecmap(trim_whitespace).parse(
            self.header_fields['MARKER NAME'])

        self.marker_number = p.Parser(n_ANY(20)).parsecmap(trim_whitespace).parse(
...
# Custom Parsec parser generators
def n_A(n):
    """Given n, return Parser for string of length n, containing only 
    alphanumeric characters
    """
    return p.regex('([A-Za-z0-9]){{{0}}}'.format(n))
